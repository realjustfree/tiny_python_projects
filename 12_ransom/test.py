...
def test_text1():
    ...
    for seed, expected in tests:
        rv, out = getstatusoutput(f'python3 {prg} {seed_flag()} {seed} "{in_text}"')
        assert rv == 0
...
def test_file1():
    ...
    for seed, expected in tests:
        rv, out = getstatusoutput(f"python3 {prg} {seed_flag()} {seed} {fox}")
        assert rv == 0
        assert out.strip() == expected
...
def test_file2():
    ...
    for seed, expected in tests:
        rv, out = getstatusoutput(f"python3 {prg} {seed_flag()} {seed} {now}")
        assert rv == 0
        assert out.strip() == expected
